...
public class StringUtil {
	...
	public static boolean isEmpty (String[] s) {
		...
		for (int i = 0; i < s.length; ++i) {
			if (!isEmptyString(s[i])) {return false;} 
		}
...
public static boolean isHKID(String idNo,String chkDig) {
	...
	public static int countChineseLength(String data){
		return 0;//ChineseUtil.countChineseLength(data);
	}
	
	public static int countNonChineseLength(String data){
		return 0;//ChineseUtil.countNonChineseLength(data);
	}
	
	public static int countEnglishLength(String data){
		int length = 0;
	...
	public static int countNonEnglishLength(String data){
		return data.length() - countEnglishLength(data);
	}
	
	public static int countAsciiLength(String data){
		int length = 0;
		if (isEmptyString(data)) {
			return length;
		} 
	...
	/**
	 * Handle the id no. For those concat idNum with check digit in SQL
	 * @param idNo
	 * @param idType
	 * @return 
	 */
	public static String handleIDNo(String idNo, String idType) {
		if(idNo==null || idType==null)	return idNo;
		idNo = idNo.trim();
		if( idType.startsWith("Z") && idNo.endsWith("()") ) { //!idNo.matches("[A-Z]{1,2}\\d{6}") || 
...
//		
//		System.out.println(StringUtil.countChineseLength("中 速度 速度萨　   2"));
//		System.out.println(StringUtil.countNonChineseLength("中 速度 速度萨　   2"));
		System.out.println(StringUtil.countEnglishLength("III"));
	}
