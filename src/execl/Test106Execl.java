...
public class Test106Execl {

	// build Excel work book
	static HSSFWorkbook workbook = new HSSFWorkbook();
	// create export file
	static String fileUniqueName = "D:/MyExec106.xls";
	static File exportFile = new File(fileUniqueName);
	static FileOutputStream fos;
	static HSSFSheet sheet = workbook.createSheet();
	private static String[] exceptionReportHeader={"Proc Unit","Case Key","Casefile reference","Assigned  Officer",
		"Applicant's name","Applicant's Identity card  number",
		"Payment Expiry Date","Pay Day"," From Date of RR",
		"TO Date of RR"," Flat No"," Block Code","Block name",
		" Estate Code","Est Name","Net Rent Payable By Tenant"," Image Date"};
	...
	public static void createHeader(){
	    ...
	    headstyle.setAlignment(headstyle.ALIGN_CENTER);// 左右居中    
	    headstyle.setVerticalAlignment(headstyle.VERTICAL_CENTER);// 上下居中    
	    headstyle.setLocked(true);    
	    headstyle.setWrapText(true);// 自动换行 
		 // 创建第一行    
	      HSSFRow rowHeader = sheet.createRow(0);  
	      HSSFCell cellHeader ;//= rowHeader.createCell(0);  
	      for(int i=0;i<exceptionReportHeader.length;i++){
	    	  
	    	  cellHeader = rowHeader.createCell(i);  
	    	  cellHeader.setCellValue(exceptionReportHeader[i]);
...
//		      
//		     /* cellEndH.setCellValue("TOTAL NUMBER OF MANUAL CHEQUE PAYMENT CASES:");
//		      cellEndH.setCellStyle(headstyleEnd); 
//		      range = new CellRangeAddress(6, 6, 0, 2);    
//		      sheet.addMergedRegion(range);
//		      //
